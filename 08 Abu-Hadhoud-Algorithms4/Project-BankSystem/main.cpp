...
stUserData ConvertUserLineToRecord(string Line, string Seperator = "/##/")
{
	...
	vUserRecord = SplitString(Line, Seperator);
...
{
	if (LoginUser.permissions == enPermissions::pAll)
		return true;
...
bool DeleteAdminRights(enPermissions permission)
{
	if (CheckAccessPermissions(permission))
	{
		system("cls");
		cout << "===========================================\n";
		cout << "     You can't delete admin\nwho has full access on the system!\n";
		cout << "===========================================\n";
		return false;
	}
...
bool UpdateAdminRights(enPermissions permission)
{
	if (CheckAccessPermissions(permission))
	{
		system("cls");
...
{
	if (!CheckAccessRights(enPermissions::pShowClientList))
	{
		return;
	}
...
{
	if (!CheckAccessRights(enPermissions::pFindClient))
	{
		return;
	}
	cout << "============================\n";
...
{
	if (!CheckAccessRights(enPermissions::pUpdateClient))
	{
		return;
	}
	cout << "============================\n";
...
{
	if (!CheckAccessRights(enPermissions::pTransactions))
	{
		return;
	}
	system("cls");
...
int ReadPremissions()
{
	int permissions = 0;
	char answer = 'n';
	cout << "\nDo you want to give full access permissions?[y/n]? ";
	cin >> answer;
	if (answer == 'y' || answer == 'Y')
	{
	...
	cout << "\nAdd new clients permissions?[y/n]? ";
	cin >> answer;
	if (answer == 'Y' || answer == 'y')
	{
	...
	cout << "\nDelete clients permissions?[y/n]? ";
	cin >> answer;
	if (answer == 'Y' || answer == 'y')
	{
...
stUserData ReadNewUser()
{
	...
	UserData.permissions = ReadPremissions();
...
bool  FindUserByUserNameAndPassword(string userSearch,string password, stUserData& user)
{
	vector <stUserData> vUser = LoadUsersDataFromFile(UsersFileName);


	for (stUserData C : vUser)
	{
		if (C.userName == userSearch && C.password == password)
		{
			user = C;
			return true;
		}
...
void PerformManageUsersOptions(enManageUsersOptions manageUsers)
{
	...
	{
		// here do nothing it will back to main menue
	}
	}
...
void ShowManageUsresMenue()
{
	if (!CheckAccessRights(enPermissions::pManageUsers))
	{
		return;
	}
	system("cls");
...
void ShowMainMenue()
{
	...
	PerformMainMenueOption((enMainMenueOptions)ReadOptions(1, 8));

}

bool IsUserExist(string userName,string password)
{
	if (FindUserByUserNameAndPassword(userName, password, LoginUser))
		return true;
	else
		return false;
}
...
void LoginScreen()
{
	...
	do
	{
		...
		loginFailed = !IsUserExist(username, password);
